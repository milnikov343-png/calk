...
# --- 2. МАТЕМАТИКА ПРЯМОУГОЛЬНИКОВ И СИММЕТРИИ (ИДЕАЛЬНАЯ А-Б-А-Б) ---
...
shape_type = st.sidebar.selectbox("Выберите конфигурацию:", ["⬜ Прямоугольная (Стандарт)", "📐 Г-образная (Угловая)", "🔲 П-образная (С вырезом)", "⏺️ Округлая (Овал / Круг)", "✏️ Свой контур (По координатам)"])
...
if shape_type == "⬜ Прямоугольная (Стандарт)":
    ...
    length = 5.0; width = 5.0; coords = [(0,0), (5,0), (5,5), (0,5)]
...
if is_complex:
    ...
    st.info("Модуль в разработке. Переключитесь на Прямоугольную форму для расчетов.")
    st.stop()
...
if use_frame:
    if "Вдоль" in direction_choice:
        front_pieces = get_shifted_edge(layout_matrix, True, offset_left, offset_right)
        back_pieces = get_shifted_edge(layout_matrix, False, offset_left, offset_right)
        left_pieces = get_1d_symmetric_pieces(width, M, min_allowed)
        right_pieces = get_1d_symmetric_pieces(width, M, min_allowed)
    else:
        left_pieces = get_shifted_edge(layout_matrix, True, offset_front, offset_back)
        right_pieces = get_shifted_edge(layout_matrix, False, offset_front, offset_back)
        front_pieces = get_1d_symmetric_pieces(length, M, min_allowed)
        back_pieces = get_1d_symmetric_pieces(length, M, min_allowed)
