...
st.sidebar.caption("Закрывает торцы, запил углов 45°.")
...
# Таблицы
mat_data = [
    {"Позиция": f"Доска террасная/торцевая: {name}", "Кол-во": f"{data['qty']} шт", "Сумма": data['sum']}
...
def get_plot(mode):
    ...
    if mode == "board":
        ...
        # Координаты для парных лаг с учетом рамки
        abs_joints = set()
        if "Вдоль" in direction_choice:
