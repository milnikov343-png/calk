...
def calculate_fence(params, prices, proflist, shtaket):
    ...
    if material_type == "Жалюзи Роял":
        jalousie_step = params.get("jalousie_step", 84)
        lamel_price_mp = prices.get("Ламель жалюзи Роял (м.п.)", 420)
        ...
        jalousie_items = [
            {"name": f"Ламель жалюзи Роял (шаг {jalousie_step}мм)",
        ...
        if n_otkatnye > 0:
            ...
            jalousie_items.append(
                {"name": f"Ламель жалюзи (ворота откатные {gate_w}м)",
        ...
        if n_raspashnye > 0:
            ...
            gate_slats_total += gate_slats_n
            jalousie_items.append(
                {"name": f"Ламель жалюзи (ворота распашные {gate_w}м)",
                 "unit": "шт", "qty": gate_slats_n, "mp": gate_mp,
                 "price_mp": lamel_price_mp, "total": round(gate_mp * lamel_price_mp)}
            )
        if n_kalitka > 0:
            ...
            jalousie_items.append(
                {"name": f"Ламель жалюзи (калитка {kal_w}м)",
    ...
    samorez_qty = math.ceil(max(total_screws, 1) / 250) * 250 if material_type != "Жалюзи Роял" else 0
...
with st.expander("🛠️ ПАРАМЕТРЫ ЗАБОРА (Нажмите, чтобы развернуть/свернуть)", expanded=True):
    ...
    with c1:
        ...
        if calc_mode == "express":
            ...
            for i in range(1, num_sides + 1):
                with st.container(border=True):
                    ...
                    s_mat_type = st.radio(f"Тип:", ["Профнастил", "Штакет", "Шахматка", "Жалюзи Роял"], horizontal=True, key=f"s_mat_type_{i}", label_visibility="collapsed")
                    if s_mat_type == "Профнастил":
                        s_mat_name = st.selectbox(f"Профлист:", list(proflist.keys()), key=f"s_mat_name_{i}")
                        s_gap = 0.0
                    elif s_mat_type == "Жалюзи Роял":
                        s_mat_name = "Жалюзи Роял полиэстер"
                        s_gap = 0.0
                    else:
        ...
        if calc_mode == "express":
            material_type = st.radio("Тип финишного материала:", ["Профнастил", "Штакет", "Шахматка", "Жалюзи Роял"], horizontal=True)

            if material_type == "Профнастил":
                material_name = st.selectbox("Выберите профлист:", list(proflist.keys()))
                gap_m = 0.0
            elif material_type == "Жалюзи Роял":
                material_name = "Жалюзи Роял полиэстер"
                jalousie_step = st.radio("Шаг ламелей:", [84, 106], horizontal=True, format_func=lambda x: f"ROYAL Z — {x} мм")
                gap_m = 0.0
            else:
...
params = {
    ...
    "jalousie_step": jalousie_step if material_type == "Жалюзи Роял" else 84
}
